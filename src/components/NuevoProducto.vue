      ...
      <b-row>
          ...
          <b-col cols="10" lg="4" md="6" sm="8">
            <b-form @reset="onReset" v-if="show" action="http://localhost:3000/NuevoProducto">
              ...
              <b-form-group>
                <b-form-select v-model="form.categoria" :options="optionsColores" @change="cambioUnInput"></b-form-select>
              </b-form-group>
...
export default {
    ...
    methods: {
      onSubmit(evt) {
        ...
        axios.post(`${this.url}NuevoProducto`, datos, config).then(function (resp) {
          // console.log(datos);
            alert(resp.data);  
            this.onReset();
        }.bind(this));
    ...
    mounted(){
      ...
      this.optionsColores.push({ 'value':null, 'text':'Categoría'});
      axios.get(`${this.url}traerColores`).then(function(resp){
        console.log(resp.data);
        resp.data.map(function(obj){
          this.optionsColores.push({ 'value':obj._id, 'text':obj.nombre })
        }.bind(this))
      }.bind(this))

    },
    computed:{
      ...mapState(['colores', 'url'])
    }
}
</script>
