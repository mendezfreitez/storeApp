<template>
  <div>
    ...
    <ModalVaciarCarro ref="modalVaciarCarro" />
  </div>
</template>

<script>
import ModalVaciarCarro from "./ModalVaciarCarro";
...
export default {
  ...
  methods: {
    ...
    procesarPago() {
      console.log(JSON.stringify(this.ProductosCarro, undefined, 4));
    },
    vaciarCarro() {
      if (this.ProductosCarro.length > 0) {
        this.$refs.modalVaciarCarro.cantElementosCarro =
          this.ProductosCarro.length;
        this.$bvModal.show("modalVaciarCarro");
      }
    },
  },
};
